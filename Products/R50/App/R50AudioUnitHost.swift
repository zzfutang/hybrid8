//
//  R50AudioUnitHost.swift
//  Standalone AUv3 host for R50: loads the extension in-process, wires it to
//  AVAudioEngine, and forwards CoreMIDI / on-screen-keyboard events to it.
//
//  This is R50's own copy rather than a shared host, so the two products can
//  diverge (different announcements, MIDI handling, performance UI) without
//  either one's changes rippling into the other.
//
...
final class R50AudioUnitHost: ObservableObject, PerformanceEventSink {
    ...
    private func load() {
        AVAudioUnit.instantiate(with: product.componentDescription, options: []) {
            ...
            // Start on a clean, deterministic patch rather than whatever macOS
            // state-restoration may have left behind.
